...
插件将使用的 DeepSeek Harness 官方模块统一声明为对等依赖，复用 Profile 提供的同一实例，避免插件私有副本与模块身份分裂；发布检查会阻止官方模块进入普通依赖。

### 子任务用量口径

Fork 子任务只统计自身产生的调用，不把从父会话继承的上下文计入自身用量；统计缓存升级时按新口径自动重建。
...
目前已在 DeepSeek Harness `0.1.1-rc.2`、Node.js `22.19+` 和 `24+` 上测试，可用于官方 Web UI，以及加载该 Web UI 的桌面封装。
